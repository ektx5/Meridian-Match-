...
with tab_form:
    ...
    with st.form("supplier_offering_form"):
        ...
        with c1:
            ...
            loc_idx = LOCATIONS.index(supplier_row["location"]) if supplier_row and supplier_row["location"] in LOCATIONS else 0
                ...
                try:
                    for m in matches:
                        if m["overall_score"] >= 60:
                            c_row = conn.execute("SELECT user_id FROM clients WHERE id=?", (m["client_id"],)).fetchone()
                            if c_row:
                                create_match_notifications(
                                    client_id=m["client_id"], supplier_id=supplier_id,
                                    match_id=m["match_id"], score=m["overall_score"],
                                    explanation=m["explanation_text"], client_user_id=c_row["user_id"],
                                    supplier_user_id=user_id,
                                )
                finally:
                    conn.close()

            st.markdown("<div class='bb-success-box'>✅ Catalog updated & AI matching complete! View matches below.</div>", unsafe_allow_html=True)
            st.rerun()
...
with tab_matches:
    if not is_complete or not supplier_row:
        st.markdown(
            ...
            "<p style='color:#52796F;'>Please submit your product offering in the form tab to find matching clients.</p>"
            "</div>",
            unsafe_allow_html=True,
        )
    else:
